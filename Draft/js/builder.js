...
function loadDailyRequests(year, month, day, filter) {
	...
	for (i = 0; i < test_data.length; i++) {
		...
		if (compareDate(currDate, item_startDate, item_endDate)) {
			...
			var item = 
				'<div class="cal-change-request-item ' + item_number.replace(" ", "-") + ' ' + item_approval.replace(" ", "-") + ' ' + item_state.replace(" ", "-") + ' ' 
					+ item_type.replace(" ", "-") + '" style="background-color:' + color + ';" onmouseover="highlightAll(\'' + item_number.toString() + '\')"  onmouseout="unhighlightAll(\'' + item_number.toString() + '\')"><a class="request-link" href="' + 
