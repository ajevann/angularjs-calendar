...
function loadDailyRequests(year, month, day) {
	...
	for (i = 0; i < test_data.length; i++) {
		if (compareDate(currDate, test_data[i][0], test_data[i][1])) {
			
			var color = setColor(test_data[i][4]);

			var item = '<div class="cal-change-request-item" style="background-color:' + color + ';"><a style="text-decoration: none;" href="' + test_data[i][6] + '">' + test_data[i][2] + ' (' + test_data[i][4] + ') ' + test_data[i][5] + '</a></div>';
			div = div.concat(item);
		}
	}
